...
const char* labels[6] = {"chopping", "door", "frying", "gas", "water", "nothing"};
...
int run_inference(float *features) {
    ...
    if (input->type == kTfLiteInt8) {
        for (int i = 0; i < 10; i++) {
            input->data.int8[i] = (int8_t)(features[i] / input->params.scale + input->params.zero_point);
        }
    } else {
        for (int i = 0; i < 10; i++) {
    ...
    // In input để debug
    Serial.print("input: ");
    for(int i=0; i<10; i++) { Serial.print(features[i]); Serial.print(" "); }
    Serial.println();
